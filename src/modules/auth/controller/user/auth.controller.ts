...
export class AuthController {
  ...
  constructor(
    ...
    this.globalPrefix = this.config.get('API_APP_PREFIX', { infer: true });
  ...
  @ApiOperation({
    summary: 'Créer un compte utilisateur et envoyer un OTP de connexion',
  })
  ...
  @ApiOperation({ summary: 'Renvoyer le mail de vérification de compte' })
  ...
  async resendVerificationEmail(@Req() req: Request) {
    try {
      ...
      await this.authService.sendVerificationEmail(
        user,
        `/${this.globalPrefix}/user/auth/verify-email`,
        lang,
      );
  ...
  @ApiOperation({
    summary: "Vérifier l'adresse e-mail d'un compte utilisateur",
  })
  ...
  @ApiOperation({ summary: 'Réinitialiser le mot de passe (ETAPES 3 et 4)' })
  @ApiQuery({
    name: 'verify',
    enum: ['on', 'off'],
    example: 'on',
    description:
      'Vérifier le code de vérification avec "on" (ETAPE 3) et créer un nouveau mot de passe avec "off" (ETAPE 4)',
  })
